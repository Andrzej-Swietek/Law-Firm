<script lang="ts">
    ...
    import {getCourtById} from "$lib/api/court/getCourtById";
    import {updateCourt} from "$lib/api/court/updateCourt";

    $: courtId = $page.params.id;


    let court: Partial<CourtDivision> = {
        id: -1,
        name: "",
        city: ""
    }
    ...
    const handleSubmit = async () => {
        try {
            const newClient = await updateCourt(courtId ,court);
            if (newClient) {
    ...
    onMount(async ()=>{
        if(isNaN(courtId)) goto("/court")

        const courtData = await getCourtById(courtId);

        if (!courtData) goto("/court");
