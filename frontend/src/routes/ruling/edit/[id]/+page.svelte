<script lang="ts">
    ...
    import {updateRuling} from "$lib/api/ruling/updateRuling";
    import {getRulingById} from "$lib/api/ruling/getRullingById";

    $: rulingId = $page.params.id;

    let ruling: Partial<Ruling> = {
        isFinal: false,
        content: "",
        trialId: 0,
        finalizationDate: "",
    ...
    onMount(async () => {
        const response = await getRulingById(rulingId);
        if (!response) goto("/ruling")
        ruling = { ...response! };
        trials = await getAllTrials();

        const [y,m,d] = ruling.finalizationDate!.split('-')
    ...
    const handleSubmit = async () => {
        ...
        try {
            const newRuling = await updateRuling(rulingId, {
                ...ruling,
                id: rulingId,
    ...
    >
        ...
        <div>
            ...
            <div class="w-full py-2">
                <Label> Currently Saved: { ruling.trial?.title } </Label>
            </div>
            <Select
