<script lang="ts">
    ...
    import {updateTrial} from "$lib/api/trial/updateTrial";

    $: trialId = $page.params.id;

    let trial: Partial<Trial> = {
        title: "",
        description: "",
        trialStatusId: -1,
        clientId: -1,
    ...
    onMount(async () => {
        try {
            ...
            selectedClientId = trial.clientId!;
            selectedLawyerId = trial.lawyerId!;
            selectedJudgeId = trial.judgeId!;
            selectedCaseId = trial.caseId!;

            const [y,m,d] = trial!.date!.split('-')
            dateValue = new CalendarDate(+y,+m,+d)
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const newTrial = await updateTrial(trialId,trial);
            if (newTrial) {
    ...
    <form class="space-y-8" on:submit|preventDefault={handleSubmit}>
        ...
        <div>
            ...
            <div class="w-full py-2">
                <Label> Currently Saved: { trial.trialStatus?.name }  </Label>
            </div>
            <Select
        ...
        <div>
            ...
            <div class="w-full py-2">
                <Label> Currently Saved: { trial.client?.firstName } { trial.client?.lastName } </Label>
            </div>
            <Select
        ...
        <div>
            ...
            <div class="w-full py-2">
                <Label> Currently Saved: { trial.lawyer?.firstName } { trial.lawyer?.lastName } </Label>
            </div>
            <Select
        ...
        <div>
            ...
            <div class="w-full py-2">
                <Label> Currently Saved: { trial.judge?.firstName } { trial.judge?.lastName } </Label>
            </div>
            <Select
        ...
        <div>
            ...
            <div class="w-full py-2">
                <Label> Currently Saved: { trial.case?.name } </Label>
                <p class="text-sm font-[400] leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                    {trial.case?.description}
                </p>
            </div>
            <Select
